...
4. **Layer 4 — server-side classifier (async).** When the bastio backend is configured with a Layer 4 engine, an excerpt (≤ 8KB) goes to it for richer PII analysis. Supported engines today: OpenAI Privacy Filter (Apache-2.0, 1.5B-total / 50M-active MoE token classifier — 97% F1 on PII-Masking-300k as published by OpenAI) and Microsoft Presidio (regex + spaCy NER fallback). The cascade falls through to a local regex + entropy heuristic when neither is configured. The local block decision has already happened in under 30ms — Layer 4 only enriches the dashboard event with higher-quality severity and reasoning. p95 budget < 500ms; responses past the budget are dropped (no UX impact).
